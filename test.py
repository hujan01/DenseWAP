...
Date: 2021-02-21 14:38:33
LastEditTime: 2021-03-06 19:49:32
'''
import math
import os 
...
# 配置参数
valid_datasets = ['data/test.pkl', 'data/CROHME2016/label/test_caption_2016.txt']
...
encoder.load_state_dict(torch.load('checkpoints/encoder_coverage.pkl'))
decoder.load_state_dict(torch.load('checkpoints/attn_decoder_coverage.pkl'))
...
for step_t, (x_t, y_t, batch_list) in enumerate(test_loader): 
    ...
    for i in range(batch_size_t):
        ...
        if len(prediction_sub)==len(label_sub) : # 计算error1, error2, error3
            e = 0
